...
Ta vừa nói Blockchain được tạo từ các Block liên kết với nhau. Hãy bắt đầu với một cấu trúc đơn giản **mỗi Block** có một **BlockID** định danh duy nhất block, một trường `PreviousBlockHash` chứa **hash của block ngay trước đó** (`Hash(Block trước)`) và Data là dữ liệu chứa trong Block - Tất nhiên, Blockchain sinh ra là để lưu dữ liệu, nên trường này là bắt buộc (Với trường hợp của BTC đó là thông tin giao dịch)
